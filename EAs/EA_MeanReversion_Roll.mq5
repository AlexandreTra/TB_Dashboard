...
//|               Bollinger + RSI + Money Management % + Rolls       |
//+------------------------------------------------------------------+
...
// ==========================================
// PARAMÈTRES DE LA STRATÉGIE (MEAN REVERSION)
// ==========================================
input group "--- Stratégie Mean Reversion ---"
input int    InpBandsPeriod  = 20;      // Période des Bandes de Bollinger
input double InpBandsDev     = 2.0;     // Déviation (Écart-type) des Bandes
input int    InpRsiPeriod    = 14;      // Période du RSI
input double InpRsiOverbought= 70.0;    // Niveau de Surachat (Vente)
input double InpRsiOversold  = 30.0;    // Niveau de Survente (Achat)
...
input long   InpMagicNumber  = 12001;
...
double bbUpper[], bbLower[], rsiBuffer[];
double closeBuffer[];
datetime RollDates[];
double SoldeInitial;
...
int OnInit()
  {
   Print("🟢 Initialisation EA Mean Reversion (BB+RSI)...");
   ...
   bbHandle  = iBands(_Symbol, PERIOD_CURRENT, InpBandsPeriod, 0, InpBandsDev, PRICE_CLOSE);
   rsiHandle = iRSI(_Symbol, PERIOD_CURRENT, InpRsiPeriod, PRICE_CLOSE);

   if(bbHandle == INVALID_HANDLE || rsiHandle == INVALID_HANDLE)
     {
      Print("❌ Erreur de création des indicateurs BB/RSI !");
      return(INIT_FAILED);
     }
   ...
   if(PositionsTotal() == 0)
     {
      ...
      double lower1 = bbLower[0];
      double upper1 = bbUpper[0];
      double rsi1   = rsiBuffer[0];

      double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);

      // -- SIGNAL D'ACHAT (Excès baissier) --
      if(close1 < lower1 && rsi1 < InpRsiOversold)
        {
         ...
         trade.Buy(lot_size, _Symbol, ask, sl_price, tp_price, "Achat MeanReversion");
        }

      // -- SIGNAL DE VENTE (Excès haussier) --
      else if(close1 > upper1 && rsi1 > InpRsiOverbought)
        {
         ...
         trade.Sell(lot_size, _Symbol, bid, sl_price, tp_price, "Vente MeanReversion");
        }
     }
  }

//+------------------------------------------------------------------+
//| HELPER : CALCUL DU LOT                                           |
//+------------------------------------------------------------------+
