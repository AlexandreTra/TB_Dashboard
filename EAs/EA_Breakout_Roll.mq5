...
//|               Price Action Breakout (Donchian) + Rolls           |
//+------------------------------------------------------------------+
...
// ==========================================
input group "--- Stratégie Breakout (Donchian) ---"
input int    InpBreakoutPeriod = 20;    // Période d'analyse (Plus haut / plus bas des N bougies)
...
input long   InpMagicNumber  = 13001;
...
int OnInit()
  {
   Print("🟢 Initialisation EA Breakout (Donchian)...");
   ...
   if(PositionsTotal() == 0)
     {
      ...
      if(CopyHigh(_Symbol, PERIOD_CURRENT, 2, InpBreakoutPeriod, highBuffer) <= 0) return;
      if(CopyLow(_Symbol, PERIOD_CURRENT, 2, InpBreakoutPeriod, lowBuffer) <= 0) return;

      int maxIdx = ArrayMaximum(highBuffer);
      int minIdx = ArrayMinimum(lowBuffer);

      double resistance = highBuffer[maxIdx];
      double support    = lowBuffer[minIdx];
      ...
      if(close1 > resistance)
        {
         ...
         trade.Buy(lot_size, _Symbol, ask, sl_price, tp_price, "Achat Breakout");
        }
      // -- CASSURE BAISSIÈRE --
      else if(close1 < support)
        {
         ...
         trade.Sell(lot_size, _Symbol, bid, sl_price, tp_price, "Vente Breakout");
        }
     }
  }
